...
std::string cmd5::calcMD5(const char* data, size_t len)
{
	std::string ret = "";
	const int MD5_SIZE = 16;
	const int MD5_STR_LEN = 2 * MD5_SIZE;
	byte md5_value[MD5_SIZE];
	char md5_str[MD5_STR_LEN];
	MD5_CTX md5;
	// init md5
	MD5Init(&md5);

	MD5Update(&md5, (byte*)data, len);

	MD5Final(&md5, (byte*)md5_value);

	// convert md5 value to md5 string
	for(int i = 0; i < MD5_SIZE; i++)
	{
		char sz[3] = {0};
		sprintf_s(sz,sizeof(sz),"%02X", md5_value[i]);
		memcpy(md5_str + i*2,sz,2);
	}
